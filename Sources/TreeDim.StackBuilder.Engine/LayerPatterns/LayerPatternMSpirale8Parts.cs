...
namespace treeDiM.StackBuilder.Engine
{
    class LayerPatternMSpirale8Parts : LayerPatternBox
    {
        ...
        public override void GenerateLayer(ILayer2D layer, double actualLength, double actualWidth)
        {
            ...
            for (int i = 0; i < noSpiraleX; ++i)
                for (int j = 0; j < noSpiraleY; ++j)
                {
                    ...
                    AddPosition(
                        layer
                        , new Vector2D(xBase, yBase + boxWidth) + offset
                        , HalfAxis.HAxis.AXIS_X_P, HalfAxis.HAxis.AXIS_Y_P);
                    // 2
                    AddPosition(
                        layer
                    ...
                    AddPosition(
                        layer
                        , new Vector2D(xBase + boxLength + 2 * boxWidth, yBase + boxLength + 2 * boxWidth) + offset
                        , HalfAxis.HAxis.AXIS_X_N, HalfAxis.HAxis.AXIS_Y_N);
                    AddPosition(
                        layer
                        , new Vector2D(xBase + boxLength + 2 * boxWidth, yBase + boxLength + boxWidth) + offset
                        , HalfAxis.HAxis.AXIS_X_N, HalfAxis.HAxis.AXIS_Y_N);
                    // 4
                    AddPosition(
                        layer
                        , new Vector2D(xBase, yBase + boxLength + 2 * boxWidth) + offset
                        , HalfAxis.HAxis.AXIS_Y_N, HalfAxis.HAxis.AXIS_X_P);
                    AddPosition(
                        layer
                        , new Vector2D(xBase + boxWidth, yBase + boxLength + 2 * boxWidth) + offset
                        , HalfAxis.HAxis.AXIS_Y_N, HalfAxis.HAxis.AXIS_X_P);
                }
            layer.UpdateMaxSpace(Math.Abs(boxLength - 2 * boxWidth), Name);
            if (area2First)
            {
                // area 2
            ...
            , out bool area2First)
        {
            ...
            return noSpiraleX * noSpiraleY * 8 + noArea2X * noArea2Y + noArea3X * noArea3Y;
        }
